...
| `showcase.html` (**The Lab**) | A lab-notebook layout: the hero is a book-style contents page (dotted leaders, prices, a hand-drawn tick for every demo you try), each chapter has a running header, a numbered spec list, a receipt-style price and a margin note on what to try, plus a sidebar that ticks off each demo you try, a window bar with a fullscreen **Focus** mode on every stage, J/K/F keyboard shortcuts, and a **Build your combo** price calculator that opens the Hire page with your picks selected. 10 interactive chapters. Seven are playable anime pages built on `assets/css/lab-manga.css` (ink gutters, halftone, speed lines, speech bubbles, SFX) and styled in `assets/css/lab-anime.css`: **Hand-Sign Dojo** (tap the twelve zodiac signs in order to cast Fireball, Chidori, Shadow Clone, Summoning, Water Dragon or Great Breakthrough), **Chakra Nature Test** (press and hold to channel chakra; the paper burns, splits, crumples, crumbles or soaks), **Summoning Scroll** (trace the seal to summon Gama, Katsuyu or the Toad Sage), **Ninja Academy Exam** (eight timed anime questions and a stamped rank scroll), **Manga Panel Maker** (layouts, characters, tones, speech bubbles and SFX, saved in the browser), **Power Scouter** (power up past 9000 through the Super Saiyan forms, then fire a Kamehameha) and **The Bingo Book** (flip-over wanted posters with stat charts, plus one for you). Three are hand-made in ink and paper: **The Block Garden** (Minecraft plugins on an isometric island), **Site Makeovers** (drag a brush line between an outdated site and its rebuild) and **The Project Scroll** (a draggable handscroll of the six project stages). Old links to the removed service chapters (`showcase#web` and friends) open that service on the Hire page. **The Motion Wing (動の間)** opens the Lab: 28 curated, responsive interface and motion studies, including a 3D product coverflow with snapping and captions, a live customers globe with order arcs, an analytics dashboard with morphing charts, a ⌘K command palette, a kanban board, 3D device mockups, 3D pricing, generative art and polished micro-interactions. Search, filters and full-screen mode; exhibits only run while on screen |
| `shop.html` (**Shop templates**) | Twelve complete store templates with real product photos: clothing, baby clothes, beauty, fine jewellery, handwoven Bangladeshi wear, bags, desk tech, film cameras, sneakers, furniture, plants and outdoor gear. Each store has its own fonts, colours, hero layout, currency (৳, $, €, £, kr), lookbook, reviews and footer. Shared engine: filters, product modal with zoom and sizes, per-store bag, promo codes, shopping assistant, checkout with bKash / Nagad / card / crypto / COD, and an admin dashboard. A sticky section bar turns every store into a whole shop, in that store's own colours, fonts and currency: **Home** (storefront), **Cart** (full page: quantities, save for later, promo codes, free-delivery meter, recommendations), **Orders** (live tracking timeline, invoice download, buy again), **Saved**, **Profile** (membership tier and points, details, addresses), **Settings** (notifications, language, size, data export/delete), **Security** (security score, password strength, two-step verification with QR and backup codes, passkeys, devices, activity log), an **AI agent** (plans and uses tools: catalogue search, bag, promo codes, order tracking; can build a set within a budget, apply the best code, track or reorder) and **Admin**. Works as a phone app layout too |
| `components.html` (**UI Kit**) | A specimen-sheet hero with redlined live components, then all 50 components (headers, bars, heroes, buttons, cards, forms, footers, loaders, alerts) as ten interactive benches, one per category: a measured stage with pink redlines (width, height, radius, child boxes), a row to pick the variant, and a token panel with canvas, accent shift, zoom, slow-motion and redline controls, live HTML / CSS / JS with copy, and present mode. A sticky category bar has a search that jumps to a component. Below it: **Anatomy** (one button with measured callouts, all six states and a token panel with a live WCAG contrast check), **Motion** (a draggable cubic-bezier editor driving real UI, with copyable CSS) and release notes. Scripts: `assets/js/components.js` and `assets/js/kit-studio.js` |
...
assets/js/lab/         one script per Lab stage; wing*.js = Motion Wing exhibits
