...
| `showcase.html` (**The Lab**) | A lab-notebook layout: the hero is a book-style contents page (dotted leaders, prices, a hand-drawn tick for every demo you try), each chapter has a running header, a numbered spec list, a receipt-style price and a margin note on what to try, plus a sidebar that ticks off each demo you try, a window bar with a fullscreen **Focus** mode on every stage, J/K/F keyboard shortcuts, and a **Build your combo** price calculator that opens the Hire page with your picks selected. 10 interactive chapters. Six are coding manga pages built on `assets/css/lab-manga.css` (ink gutters, halftone, speed lines, speech bubbles, SFX): **The Build Arc** (websites: hand-sign buttons add sections while the code types itself and the page is inked, then Deploy), **Shadow Clone Cron** (automation: script clones run overnight and hand in a morning report), **Pop-up Demon Slayer** (Chrome extension: block pop-ups, highlight words, watch a price), **The Installer Arc** (desktop EXE: download, install, license key, launch the app), **Phone Jutsu** (mobile app: order food and follow the rider across the panels) and **Ask Sensei** (AI chat in English and Bangla with order tracking and human handover). Four are hand-made in ink and paper: **The Block Garden** (Minecraft plugins on an isometric island), **Site Makeovers** (drag a brush line between an outdated site and its rebuild), **The Project Scroll** (a draggable handscroll of the six project stages) and **The Wish Wall** (ema plaques that turn over into a plan and a price; a wish pre-fills the Hire brief). **The Motion Wing (動の間)** opens the Lab: 28 curated, responsive interface and motion studies, including a 3D product coverflow with snapping and captions, a live customers globe with order arcs, an analytics dashboard with morphing charts, a ⌘K command palette, a kanban board, 3D device mockups, 3D pricing, generative art and polished micro-interactions. Search, filters and full-screen mode; exhibits only run while on screen |
