...
| `components.html` (**UI Kit**) | A specimen-sheet hero with redlined live components, then a Figma-style workspace with 50 live components (headers, bars, heroes, buttons, cards, forms, footers, loaders, alerts): layers panel, zoom/pan, pixel rulers that track the selection, two collaborator cursors, an inspector that reads real computed styles, copyable HTML/CSS/JS and present mode. Below it: **Anatomy** (one button with measured callouts, all six states and a token panel with a live WCAG contrast check), **Motion** (a draggable cubic-bezier editor driving real UI, with copyable CSS) and release notes. Extras live in `assets/js/kit-studio.js` |
