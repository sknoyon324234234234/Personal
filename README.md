...
| `showcase.html` (**The Lab**) | A lab-notebook layout: the hero is a book-style contents page (dotted leaders, prices, a hand-drawn tick for every demo you try), each chapter has a running header, a numbered spec list, a receipt-style price and a margin note on what to try, plus a sidebar that ticks off each demo you try, a window bar with a fullscreen **Focus** mode on every stage, J/K/F keyboard shortcuts, and a **Build your combo** price calculator that opens the Hire page with your picks selected. 10 interactive chapters: live website preview, drag-and-drop automation workflow, Chrome extension popup that edits a page, **BlockRealm**, a playable Minecraft-style survival game (see below), desktop OS with an EXE installer and app, Android app, AI chat widget (English + Bangla), plus three hand-made chapters in ink and paper: **Site Makeovers** (drag a brush line between an outdated site and its rebuild while load time, page weight and Lighthouse score change), **The Project Scroll** (a draggable handscroll that paints the six stages of a project and stamps a seal at each) and **The Wish Wall** (wooden ema plaques that turn over into a plan and a price; visitors can hang their own wish, which pre-fills the Hire brief). **The Motion Wing (動の間)** opens the Lab: 28 curated, responsive interface and motion studies, including a 3D product coverflow with snapping and captions, a live customers globe with order arcs, an analytics dashboard with morphing charts, a ⌘K command palette, a kanban board, 3D device mockups, 3D pricing, generative art and polished micro-interactions. Search, filters and full-screen mode; exhibits only run while on screen |
