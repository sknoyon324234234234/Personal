...
(function () {
  ...
  /* site content: the chosen website's copy and photos, falling back to Lumen's */
  function C(k, d) { var c = ctx && ctx.site && ctx.site.c; return c && c[k] != null ? c[k] : d; }
  function pics() { return (ctx && ctx.site && ctx.site.img) || null; }
  function src(p) { try { return new URL(p, document.baseURI).href; } catch (e) { return p; } }
  function photo(i, cls) { var L = pics(); return L ? '<img class="ph' + (cls ? ' ' + cls : '') + '" src="' + src(L[i % L.length]) + '" alt="" loading="lazy">' : ''; }
  function collage() {
    var hs = C('heroStats', [['', '']])[0];
    return '<div class="pcol">' + photo(0, 'p0') + photo(1, 'p1') + photo(2, 'p2') + '<div class="float f1 card">' + ic('spark') + '<span><b>' + esc(hs[0]) + '</b><small>' + esc(hs[1]) + '</small></span></div><div class="float f2 card"><span class="dot"></span>' + esc(C('proof', '')) + '</div></div>';
  }
  ...
  SEC.nav = { name: 'Navbar', v: ['Classic', 'Centered', 'Floating pill', 'Minimal', 'Announcement'], r: function (v) {
    var links = '<nav class="links">' + C('links', LINKS).map(function (l) { return '<a href="#">' + l + '</a>'; }).join('') + '</nav>';
    ...
    var mob = '<div class="mnav">' + C('links', LINKS).map(function (l) { return '<a href="#">' + l + '</a>'; }).join('') + '<a href="#">Log in</a></div>';
  ...
  SEC.hero = { name: 'Hero', v: ['Centered', 'Split + product', 'Editorial', 'Gradient + stats', 'Search'], r: function (v) {
    var S = ctx.S, h1 = S.tag, sub = C('sub', 'Plan, build and ship with one calm workspace. ' + S.brand + ' keeps your team, your data and your customers in the same place.'), c1 = C('cta1', ''), c2 = C('cta2', '');
    var proof = '<div class="proof"><div class="avs">' + [0, 1, 2, 3].map(av).join('') + '</div><span><b>' + star() + star() + star() + star() + star() + '</b>' + esc(C('proof', 'Loved by 12,000+ teams')) + '</span></div>';
    if (v === 1) return '<section class="s hero hero-split"><div class="wrap grid2">' + '<div>' + eyebrow(C('eb', 'Now with AI workflows')) + '<h1>' + esc(h1) + '</h1><p class="lead">' + esc(sub) + '</p><div class="btns">' + B(c1 || 'Start free trial') + B(c2 || 'Watch demo', 'sec') + '</div>' + proof + '</div>' + (pics() ? collage() : mock()) + '</div></section>';
    if (v === 2) return '<section class="s hero hero-ed"><div class="wrap"><h1 class="huge">' + esc(h1) + '</h1><div class="ed-row"><p class="lead">' + esc(sub) + '</p><div class="btns">' + B(c1 || 'Get started') + B(c2 || 'See pricing', 'sec') + '</div></div><div class="ed-img">' + (pics() ? '<div class="ed-ph">' + photo(0) + '</div>' : '<div class="ed-art"><i></i><i></i><i></i></div>') + '<span class="card tagc">' + ic('spark') + esc(C('eb', 'Designed for focus')) + '</span></div></div></section>';
    if (v === 3) return '<section class="s hero hero-g"><div class="mesh"></div><div class="wrap center">' + eyebrow(C('eb', 'Trusted by 12,000+ teams')) + '<h1>' + esc(h1) + '</h1><p class="lead">' + esc(sub) + '</p><div class="btns jc">' + B(c1 || 'Start for free') + B(c2 || 'Talk to sales', 'sec') + '</div><div class="stats3">' + C('heroStats', [['99.99%', 'Uptime'], ['38 ms', 'Median latency'], ['4.9/5', 'Average rating']]).map(function (x) { return '<div class="card glass"><b>' + x[0] + '</b><span>' + x[1] + '</span></div>'; }).join('') + '</div></div></section>';
    if (v === 4) return '<section class="s hero hero-s"><div class="wrap center">' + eyebrow(C('eb', '2,400+ templates and guides')) + '<h1>' + esc(h1) + '</h1><p class="lead">' + esc(sub) + '</p><form class="sbar card" data-form>' + ic('search') + '<input placeholder="Search templates, guides, people" aria-label="Search">' + '<button class="btn pri" type="submit"><span>Search</span></button></form><div class="tags"><span>Popular:</span>' + ['Onboarding', 'Roadmaps', 'CRM', 'Invoices', 'Hiring'].map(function (t) { return '<a href="#">' + t + '</a>'; }).join('') + '</div></div></section>';
    return '<section class="s hero hero-c"><div class="wrap center">' + eyebrow(C('eb', 'New: AI workflows are live')) + '<h1>' + esc(h1) + '</h1><p class="lead">' + esc(sub) + '</p><div class="btns jc">' + B(c1 || 'Start free trial') + B(c2 || 'Book a demo', 'sec') + '</div>' + proof + (pics() ? '<div class="hstrip">' + [0, 1, 2, 3].map(function (i) { return photo(i); }).join('') + '</div>' : '') + '</div></section>';
  } };

  SEC.logos = { name: 'Logo cloud', v: ['Strip', 'Cards'], r: function (v) {
    var L = C('logos', LOGOS).map(function (l) { return '<span class="wm" style="font-weight:' + l[1] + ';' + (l[2] ? 'font-style:italic;' : '') + '">' + l[0] + '</span>'; }).join('');
    if (v === 1) return '<section class="s logos"><div class="wrap"><p class="kick center">' + esc(C('logosH', 'Teams that ship with ' + ctx.S.brand)) + '</p><div class="lgrid">' + C('logos', LOGOS).map(function (l) { return '<div class="card"><span class="wm" style="font-weight:' + l[1] + '">' + l[0] + '</span></div>'; }).join('') + '</div></div></section>';
    return '<section class="s logos sm"><div class="wrap"><p class="kick center">' + esc(C('logosH', 'Trusted by fast-moving teams')) + '</p><div class="lstrip">' + L + '</div></div></section>';
  } };
  ...
  SEC.features = { name: 'Features', v: ['Card grid', 'Alternating rows', 'Bento'], r: function (v) {
    var FEAT = C('feat', FEAT0), ft = C('featH', '');
    var head = function (t) { return '<div class="shead center">' + eyebrow('Features') + '<h2>' + esc(ft || t) + '</h2><p class="lead">' + esc(C('featSub', 'Everything you need, nothing you will not use.')) + '</p></div>'; };
    if (v === 1) return '<section class="s feat-alt"><div class="wrap">' + head('Work the way your team thinks') + FEAT.slice(0, 3).map(function (f, i) {
      return '<div class="frow' + (i % 2 ? ' rev' : '') + '"><div><span class="fic">' + ic(f[0]) + '</span><h3>' + esc(f[1]) + '</h3><p>' + esc(f[2]) + '</p><ul class="ticks"><li>' + ic('check') + 'Set up in minutes</li><li>' + ic('check') + 'No code required</li><li>' + ic('check') + 'Works on every device</li></ul></div><div class="vis card">' + (pics() ? photo(i + 1, 'fill') : '') + '<div class="vis-in v' + i + '"' + (pics() ? ' hidden' : '') + '>' + (i === 0 ? '<b></b><b></b><b></b><b></b>' : i === 1 ? '<span></span><span></span><span></span>' : '<i></i><i></i><i></i><i></i><i></i><i></i>') + '</div></div></div>';
    }).join('') + '</div></section>';
    if (v === 2 && pics()) return '<section class="s feat-bento"><div class="wrap">' + head('') + '<div class="bento">' +
      '<div class="card b1 phc">' + photo(3) + '<div><span class="fic">' + ic(FEAT[0][0]) + '</span><h3>' + esc(FEAT[0][1]) + '</h3><p>' + esc(FEAT[0][2]) + '</p></div></div>' +
      '<div class="card b2 inv"><span class="fic">' + ic(FEAT[1][0]) + '</span><h3>' + esc(FEAT[1][1]) + '</h3><p class="big">' + esc(C('stats', [['', '']])[0][0]) + '</p><p>' + esc(C('stats', [['', '']])[0][1]) + '</p></div>' +
      '<div class="card b3"><span class="fic">' + ic(FEAT[2][0]) + '</span><h3>' + esc(FEAT[2][1]) + '</h3><p>' + esc(FEAT[2][2]) + '</p></div>' +
      '<div class="card b4"><span class="fic">' + ic(FEAT[3][0]) + '</span><h3>' + esc(FEAT[3][1]) + '</h3><p>' + esc(FEAT[3][2]) + '</p></div>' +
      '<div class="card b5 phc">' + photo(4) + '<div><span class="fic">' + ic(FEAT[4][0]) + '</span><h3>' + esc(FEAT[4][1]) + '</h3><p>' + esc(FEAT[4][2]) + '</p></div></div>' +
      '</div></div></section>';
  ...
  SEC.stats = { name: 'Stats', v: ['Numbers row', 'Color band'], r: function (v) {
    var st = C('stats', [['12k+', 'Teams onboard'], ['99.99%', 'Uptime, last 12 months'], ['4.2x', 'Faster shipping'], ['38 ms', 'Median latency']]);
  ...
  SEC.testimonials = { name: 'Testimonials', v: ['Cards', 'Big quote', 'Marquee'], r: function (v) {
    ...
    if (v === 2) return '<section class="s tmq"><div class="shead center wrap">' + eyebrow('Wall of love') + '<h2>' + esc(C('quoteH', 'People say nice things')) + '</h2></div><div class="mq"><div class="mq-in">' + cards.concat(cards).join('') + '</div></div></section>';
    return '<section class="s tcards"><div class="wrap"><div class="shead center">' + eyebrow('Customers') + '<h2>' + esc(C('quoteH', 'Teams are shipping more, and meeting less')) + '</h2></div><div class="g3">' + cards.join('') + '</div></div></section>';
  } };

  var PLANS0 = [['Starter', 0, 0, 'For side projects and trying things out.', ['Up to 3 projects', 'Community support', '1 GB storage']], ['Pro', 24, 19, 'For growing teams that ship every week.', ['Unlimited projects', 'Priority support', '100 GB storage', 'Advanced analytics']], ['Business', 79, 63, 'For companies with serious needs.', ['Everything in Pro', 'SSO and audit logs', 'Dedicated manager', '99.99% SLA']]];
  SEC.pricing = { name: 'Pricing', v: ['Three cards', 'Toggle + cards', 'Compare table'], r: function (v) {
    var PLANS = C('plans', PLANS0), per = C('per', '/month');
    var head = '<div class="shead center">' + eyebrow('Pricing') + '<h2>' + esc(C('priceH', 'Simple pricing. No surprises.')) + '</h2><p class="lead">' + esc(C('priceSub', 'Start free. Upgrade when you grow. Cancel any time.')) + '</p>' + (v === 1 ? '<div class="tgl" data-tgl><button type="button" class="on" data-per="m">Monthly</button><button type="button" data-per="y">Yearly <em>-20%</em></button></div>' : '') + '</div>';
    if (v === 2) {
      ...
      return '<section class="s pricing"><div class="wrap">' + head + '<div class="card tblw"><table class="cmp"><thead><tr><th></th>' + PLANS.map(function (p, i) { return '<th' + (i === 1 ? ' class="hl"' : '') + '><b>' + p[0] + '</b><span class="pr">$' + p[1] + '<small>' + per + '</small></span>' + B(i === 0 ? 'Start free' : 'Choose ' + p[0], i === 1 ? 'pri' : 'sec') + '</th>'; }).join('') + '</tr></thead><tbody>' +
    ...
    return '<section class="s pricing"><div class="wrap">' + head + '<div class="g3 plans">' + PLANS.map(function (p, i) {
      return '<div class="card plan' + (i === 1 ? ' pop' : '') + '">' + (i === 1 ? '<span class="badge">Most popular</span>' : '') + '<h3>' + p[0] + '</h3><p>' + p[3] + '</p><div class="pr"><b data-m="$' + p[1] + '" data-y="$' + p[2] + '">$' + p[1] + '</b><small>' + per + '</small></div>' + B(i === 0 ? 'Start free' : 'Choose ' + p[0], i === 1 ? 'pri' : 'sec') + '<ul class="ticks">' + p[4].map(function (f) { return '<li>' + ic('check') + f + '</li>'; }).join('') + '</ul></div>';
    }).join('') + '</div></div></section>';
  } };

  var FAQ0 = [['Can I try it before I pay?', 'Yes. Every plan starts with a 14-day trial, and the Starter plan is free forever.'], ['Do you offer discounts for students?', 'Students and non-profits get 50% off any paid plan. Just email us from your school or org address.'], ['Can I change plans later?', 'Any time. Upgrades apply instantly and downgrades apply at the end of your billing cycle.'], ['Where is my data stored?', 'In the region you choose: EU, US or Asia. Backups are encrypted and kept for 30 days.'], ['Do you accept crypto?', 'Yes. We accept USDT, BTC and ETH on yearly plans, as well as all major cards.']];
  ...
  SEC.cta = { name: 'Call to action', v: ['Banner', 'Split + email', 'Minimal'], r: function (v) {
    if (v === 1) return '<section class="s cta2"><div class="wrap"><div class="card grid2 ctab"><div><h2>' + esc(C('ctaT', 'Get the product update, once a month')) + '</h2><p class="lead">' + esc(C('ctaS', 'New features, customer stories and zero spam.')) + '</p></div><form class="nl" data-form><input type="email" placeholder="you@company.com" aria-label="Email" required><button class="btn pri" type="submit"><span>Subscribe</span></button></form></div></div></section>';
    if (v === 2) return '<section class="s cta3"><div class="wrap center"><h2>' + esc(C('ctaT', 'Ready when you are.')) + '</h2><p class="lead">' + esc(C('ctaS', 'Set up in five minutes. No card needed.')) + '</p><div class="btns jc">' + B(C('cta1', 'Start free trial')) + '</div></div></section>';
    return '<section class="s cta1"><div class="wrap"><div class="banner"><div><h2>' + esc(C('ctaT', 'Start building with ' + ctx.S.brand + ' today')) + '</h2><p>' + esc(C('ctaS', 'Join 12,000+ teams. Free for 14 days, cancel in one click.')) + '</p></div><div class="btns">' + B(C('cta1', 'Start free trial')) + B(C('cta2', 'Talk to sales'), 'sec') + '</div><div class="ring r1"></div><div class="ring r2"></div></div></div></section>';
  } };
  ...
  SEC.footer = { name: 'Footer', v: ['Columns', 'Simple', 'Big brand'], r: function (v) {
    ...
    if (v === 1) return '<footer class="foot f1"><div class="wrap bar">' + logo() + '<nav class="links">' + C('links', LINKS).map(function (l) { return '<a href="#">' + l + '</a>'; }).join('') + '</nav><small>© ' + yr + ' ' + brand + '</small></div></footer>';
    if (v === 2) return '<footer class="foot f2"><div class="wrap"><div class="bar"><p class="lead">' + esc(C('footLine', 'Made for teams who would rather be building.')) + '</p>' + soc + '</div><div class="giant">' + brand + '</div><div class="bar small"><small>© ' + yr + ' ' + brand + ' Inc.</small><nav class="links"><a href="#">Privacy</a><a href="#">Terms</a><a href="#">Status</a></nav></div></div></footer>';
  ...
  SEC.gallery = { name: 'Portfolio', v: ['Masonry', 'Uniform grid', 'Hover list'], r: function (v) {
    var PROJ = C('proj', PROJ0).map(function (p, i) { return [p[0], p[1], p[2] || (ctx.site ? '' : (PROJ0[i] || PROJ0[0])[2])]; });
    var cats = ['All'].concat(PROJ.map(function (p) { return p[1]; }).filter(function (c, i, a) { return a.indexOf(c) === i; }).slice(0, 4));
    var head = '<div class="shead gh"><div>' + eyebrow('Selected work') + '<h1>' + esc(C('projH', 'Work that ' + ctx.S.brand + ' is proud of')) + '</h1><p class="lead">' + esc(C('projSub', 'Brands, products and motion for teams that care about the details.')) + '</p></div><div class="gf">' + cats.map(function (c, i) { return '<button type="button" data-gf="' + c + '" class="' + (i ? '' : 'on') + '">' + c + '</button>'; }).join('') + '</div></div>';
    ...
    return '<section class="s gal' + (v === 1 ? ' gl2' : '') + '"><div class="wrap">' + head + '<div class="' + (v === 1 ? 'g3' : 'mas') + '">' + PROJ.map(function (p, i) { return '<a href="#" class="card pj" data-cat="' + p[1] + '" data-lb="' + i + '">' + pa(i) + '<span class="pjb"><b>' + p[0] + '</b><small>' + p[1] + (p[2] ? ' · ' + p[2] : '') + '</small></span></a>'; }).join('') + '</div></div>' +
      LBOX() + '</section>';
  } };
  ...
  SEC.product = { name: 'Product', v: ['Gallery + details', 'Centered', 'Split sticky'], r: function (v) {
    var PR = C('prod', { name: 'Aura One', desc: 'Wireless noise-cancelling headphones with 60 hours of battery and a sound you can feel.', price: '$249', was: '$299', img: 0 }), pi = PR.img || 0;
    var sw = '<div class="opt"> <span>' + (ctx.site ? 'Colour' : 'Finish') + ': <b data-swn>' + SW[0][0] + '</b></span><div class="sws">' + SW.map(function (s, i) { return '<button type="button" data-sw="' + s[1] + '" data-swl="' + s[0] + '" class="' + (i ? '' : 'on') + '" style="background:' + s[1] + '" aria-label="' + s[0] + '"></button>'; }).join('') + '</div></div>';
    var size = ctx.site ? '<div class="opt"><span>Packaging</span><div class="seg2"><button type="button" class="on">Standard</button><button type="button">Gift wrap +$5</button></div></div>' : '<div class="opt"><span>Case</span><div class="seg2"><button type="button" class="on">Soft pouch</button><button type="button">Hard case +$19</button></div></div>';
    ...
    var info = '<p class="rate">' + star() + star() + star() + star() + star() + '<span>4.9 · 2,318 reviews</span></p><h1>' + esc(PR.name) + '</h1><p class="lead">' + esc(PR.desc) + '</p><div class="price"><b>' + esc(PR.price) + '</b><s>' + esc(PR.was) + '</s><em>' + (ctx.site ? 'Save today' : 'Save 17%') + '</em></div>' + sw + size + buy +
      '<ul class="perks"><li>' + ic('bolt') + 'Free 2-day delivery</li><li>' + ic('shield') + '2-year warranty</li><li>' + ic('back') + '30-day returns</li></ul>' +
      '<details class="acc card" open><summary>What is in the box' + ic('plus') + '</summary><p>' + esc(PR.box || (ctx.site ? 'Your ' + PR.name + ', gift-wrapped by hand, with a care card and a note from the ' + ctx.S.brand + ' team.' : 'Aura One headphones, USB-C cable, 3.5mm cable, pouch and a very small manual.')) + '</p></details><details class="acc card"><summary>' + (ctx.site ? 'Details and care' : 'Specs') + ic('plus') + '</summary><p>' + esc(PR.spec || (ctx.site ? PR.desc + ' Questions? Our team replies within the hour.' : '40mm drivers · Bluetooth 5.4 · ANC with 6 mics · 250g · 60h battery (40h with ANC).')) + '</p></details>';
    var thumbs = '<div class="thumbs">' + [0, 1, 2, 3].map(function (i) { return '<button type="button" data-th="' + i + '" class="' + (i ? '' : 'on') + '" aria-label="View angle ' + (i + 1) + '">' + (pics() ? photo(pi + i) : phones()) + '</button>'; }).join('') + '</div>';
    var hero = '<div class="pimg' + (pics() ? ' has-ph' : '') + '" data-pimg>' + (pics() ? photo(pi) : phones(true)) + '<span class="badge2">New</span></div>';
  ...
  var CSS = [
    ...
    /* photos from the chosen website */
    ".ph{display:block;width:100%;height:100%;object-fit:cover}",
    ".pcol{position:relative;display:grid;grid-template-columns:1.25fr 1fr;grid-template-rows:1fr 1fr;gap:14px;aspect-ratio:1.08;min-height:360px}.pcol .ph{border-radius:var(--r);box-shadow:0 30px 60px -30px rgba(0,0,0,.45)}.pcol .p0{grid-row:1/3}.pcol .float{position:absolute;display:flex;align-items:center;gap:10px;padding:12px 16px;font-size:13px;z-index:2}.pcol .float b{display:block;font-size:15px}.pcol .float small{color:var(--mt)}.pcol .f1{left:-22px;bottom:14%}.pcol .f2{right:-14px;top:8%}.pcol .dot{width:8px;height:8px;border-radius:50%;background:#22c55e;box-shadow:0 0 0 4px rgba(34,197,94,.2)}",
    ".ed-ph{position:relative;overflow:hidden;border-radius:var(--r);aspect-ratio:21/9}.ed-ph .ph{position:absolute;inset:0}",
    ".hstrip{display:grid;grid-template-columns:repeat(4,1fr);gap:14px;margin-top:44px}.hstrip .ph{aspect-ratio:3/4;border-radius:var(--r)}.hstrip .ph:nth-child(even){transform:translateY(28px)}",
    ".phc{position:relative;overflow:hidden;display:grid;align-content:end;min-height:260px;color:#fff}.phc>.ph{position:absolute;inset:0;z-index:0}.phc>div{position:relative;z-index:1;padding-top:40px;background:linear-gradient(180deg,transparent,rgba(0,0,0,.72))}.phc p{color:rgba(255,255,255,.85)}.phc .fic{background:rgba(255,255,255,.18);color:#fff}",
    ".vis{position:relative;overflow:hidden}.vis .ph.fill{position:absolute;inset:0}",
    ".pa.has-ph{background:var(--sf)}.pa.has-ph .ph{position:absolute;inset:0;transition:transform .8s cubic-bezier(.2,.8,.2,1)}.pj:hover .pa.has-ph .ph{transform:scale(1.06)}",
    ".pimg.has-ph{overflow:hidden;background:var(--sf)}.pimg.has-ph .ph{position:absolute;inset:0}.thumbs button .ph{border-radius:inherit}",
    "@media(max-width:760px){.pcol{min-height:280px}.pcol .f1{left:8px}.pcol .f2{right:8px}.hstrip{grid-template-columns:repeat(2,1fr)}}"
  ].join('\n');
  ...
  function body(S, page) {
    ctx = { S: S, site: (window.XRSites || []).filter(function (x) { return x.id === S.site && x.c; })[0] || null };
